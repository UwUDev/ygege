...
Francisca is available as an official multi-architecture Docker image. This guide explains how to deploy and configure the service.
...
The Francisca Docker image runs by default with a non-root user (UID 10001) for security reasons. This ensures:
...
```yaml
volumes:
  - francisca_sessions:/app/sessions  # Automatic permission management
```
...
services:
  francisca:
    ...
    volumes:
      - ./francisca/sessions:/app/sessions
```
...
**Linux/macOS**:
```bash
sudo chown -R 10001:10001 ./francisca/sessions
sudo chmod -R 755 ./francisca/sessions
```

**Windows** (PowerShell as Administrator):
```powershell
icacls ".\francisca\sessions" /grant Everyone:(OI)(CI)F /T
```
