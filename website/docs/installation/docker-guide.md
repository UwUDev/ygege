...
Francisca est disponible sous forme d'image Docker officielle multi-architecture. Ce guide explique comment déployer et configurer le service.
...
L'image Docker Francisca s'exécute par défaut avec un utilisateur non-root (UID 10001) pour des raisons de sécurité. Cela garantit:
...
Francisca ne nécessite pas de volume persistant pour les sessions puisque le tracker est public.
