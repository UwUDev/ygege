...
# Compiler Francisca depuis les sources

Ce guide explique comment compiler Francisca depuis les sources sur différentes plateformes.
...
```bash
# Cloner le dépôt
git clone https://github.com/UwUDev/francisca.git
cd francisca

# Compiler en mode release
cargo build --release

# Le binaire sera dans : target/release/francisca (ou francisca.exe sur Windows)
```
...
upx --best --lzma target/release/francisca
```
...
docker build -f docker/Dockerfile -t francisca:local .
...
Après la compilation, vous pouvez exécuter Francisca :

```bash
# Afficher la version
./target/release/francisca --version

# Exécuter avec les paramètres par défaut (voir README.md pour la configuration)
./target/release/francisca
```
...
Consultez la page [Releases](https://github.com/UwUDev/francisca/releases) pour les binaires pré-compilés.
