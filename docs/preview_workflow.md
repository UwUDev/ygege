...
The preview workflow allows contributors and reviewers to test pull requests without manually building Docker images.
When a PR is labeled with `preview`, a Docker image is automatically built and published for easy testing.
...
- `uwucode/francisca:pr-123` - Simple PR number tag (updates on every push)
- `uwucode/francisca:pr-123-feature-awesome` - Includes branch name
- `ghcr.io/uwudev/francisca:pr-123` - GHCR version
- `ghcr.io/uwudev/francisca:pr-123-feature-awesome` - GHCR with branch
...
**Problem**: `docker pull uwucode/francisca:pr-123` fails
...
**Workaround**: Use tags manually: `uwucode/francisca:pr-{number}`
...
docker pull uwucode/francisca:pr-111  # Feature A
docker pull uwucode/francisca:pr-222  # Feature B
...
# Usage
francisca-pr123  # Runs PR #123 preview
```
