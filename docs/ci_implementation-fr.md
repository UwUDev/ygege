...
# Guide du Pipeline CI/CD Francisca

> **Démarrage rapide :** Ce document explique comment Francisca est automatiquement compilé, testé et distribué.
> Utilisez-le comme référence pour dépanner les problèmes de build ou comprendre le processus de release.

## 📋 Table des matières

- [Ce qui est compilé automatiquement](#ce-qui-est-compilé-automatiquement)
- [Comment obtenir Francisca](#comment-obtenir-francisca)
...
À chaque fois que du code est poussé sur GitHub, notre système automatisé compile Francisca pour plusieurs plateformes :
...
**Qu'est-ce qu'UPX ?** Les versions compressées sont 50-70% plus petites mais mettent légèrement plus de temps à
démarrer. Utilisez la version normale si vous n'êtes pas sûr.
...
docker pull uwucode/francisca:develop

# Version spécifique
docker pull uwucode/francisca:0.4.2
```

**Registre alternatif (GitHub) :**

```bash
docker pull ghcr.io/uwudev/francisca:latest
```
...
1. Allez sur [GitHub Actions](https://github.com/UwUDev/francisca/actions)
...
4. Téléchargez le fichier correspondant à votre système :
    - **Linux (la plupart des systèmes) :** `francisca-linux-gnu-x86_64.zip`
    - **Linux (ARM/Raspberry Pi) :** `francisca-linux-gnu-aarch64.zip` ou `francisca-linux-gnu-armv7.zip`
    - **Windows :** `francisca-windows-x86_64.zip`
    - **macOS (Intel) :** `francisca-macos-x86_64.zip`
    - **macOS (Apple Silicon) :** `francisca-macos-aarch64.zip`

**Vous voulez des fichiers plus petits ?** Cherchez les versions `-upx` (ex : `francisca-linux-gnu-x86_64-upx.zip`)

**Note :** Les artifacts sont conservés pendant 7 jours. Pour des releases permanentes, utilisez les images Docker ou
attendez une release taguée.
...
Chaque binaire Francisca inclut des informations de build :
...
Quand vous démarrez Francisca, il enregistre automatiquement les infos de version :

```
INFO Francisca v0.4.2 (commit: a1b2c3d, branch: develop, built: 2025-11-11T10:30:00Z)
INFO Connected to relay relay.example.org
```
...
**Solution :** Vérifiez la [section Artifacts](#option-2--télécharger-les-binaires-💾) dans GitHub Actions. Nous
compilons pour :
...
Pas dans la liste ? Ouvrez une [issue](https://github.com/UwUDev/francisca/issues) pour demander votre plateforme.

### ❓ "Le pull Docker échoue ou l'image est introuvable"

**Vérifiez :**

1. Orthographe : `uwucode/francisca` (pas `uwudev`)
2. Le tag existe : `develop`, `latest`, `stable`, ou numéro de version
3. Essayez le registre alternatif : `ghcr.io/uwudev/francisca:latest`

**Exemple d'erreur :**

```
Error: manifest for uwucode/francisca:wrong-tag not found
```
...
2. **Info de version :** Sortie de `./francisca --version`
...
Version: Francisca v0.4.2 (commit: a1b2c3d, branch: develop)
Source: Docker uwucode/francisca:develop
...
Visualiser avec : `docker inspect uwucode/francisca:latest`
...
**Vous avez trouvé un bug ?** [Ouvrez une issue](https://github.com/UwUDev/francisca/issues)
