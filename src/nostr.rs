...
/// Probe the given relays and return them ordered by latency (fastest first).
/// Relays come from configuration; nothing is hardcoded.
pub async fn rank_relays(relays: &[String], use_tor: bool, tor_proxy: Option<&str>) -> Vec<String> {
    let timeout_dur = match tor_proxy {
        Some(_) if use_tor => Duration::from_secs(20),
        _ => Duration::from_secs(5),
    };

    let mut futures_set: FuturesUnordered<_> = relays
        .iter()
        .map(|url| {
...
impl NostrClient {
    ...
    async fn remove_first_relay(&self) -> bool {
        ...
        let fresh = rank_relays(&self.all_relays, self.use_tor, self.tor_proxy.as_deref()).await;
        if fresh.is_empty() {
    ...
    pub async fn search(
        ...
        let torrents = events
            .into_iter()
            .filter_map(|e| parse_nip35_event(e, self.web_base_url.as_deref()))
            .collect();
        Ok(torrents)
    }
                                    ...
                                    Some("EVENT") => {
                                        if arr.get(1).and_then(|v| v.as_str()) == Some(sub_id) {
                                            if let Some(event) = arr.get(2) {
                                                ...
                                                if !self.allowed_pubkeys.is_empty() && !self.allowed_pubkeys.iter().any(|p| p == pubkey) {
                                                    debug!(
                                                        "Dropped event from non-allowed pubkey: {}",
                                                        pubkey
                                                    );
...
fn parse_nip35_event(event: Value, web_base_url: Option<&str>) -> Option<Torrent> {
    ...
    // Seed/leech/completed are stored in "l" tags with "u2p." prefixes
    ...
    // Build magnet link from a fixed list of public trackers
    ...
    let link = match web_base_url {
        Some(base) => format!("{}/#/torrent/{}", base.trim_end_matches('/'), event_id),
        None => String::new(),
    };

    // Prefer published_at tag over event created_at
...
pub fn magnet_from_event(event: &Value) -> Option<String> {
    parse_nip35_event(event.clone(), None).map(|t| t.magnet)
}
