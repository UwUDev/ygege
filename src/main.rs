...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    pretty_env_logger::formatted_builder()
        ...
        .filter_module("francisca", config.log_level)
        .init();

    info!(
        "Francisca v{} (commit: {}, branch: {}, built: {})",
    ...
    if config.relays.is_empty() {
        error!(
            "No Nostr relays configured. Set the `relays` array in config.json or the RELAYS environment variable (comma-separated wss:// / ws:// URLs). Exiting."
        );
        std::process::exit(1);
    }
    ...
    let ranked_relays =
        rank_relays(&config.relays, config.use_tor, config.tor_proxy.as_deref()).await;
    if ranked_relays.is_empty() {
        error!(
            "No configured Nostr relays are reachable, try again later or check your network connection. Exiting."
        );
        std::process::exit(1);
    }
    info!(
