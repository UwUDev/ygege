...
fn load_config_from_env() -> Result<Config, std::io::Error> {
    ...
    let allowed_pubkeys = std::env::var("ALLOWED_PUBKEYS")
        ...
        .unwrap_or_else(default_allowed_pubkeys);

    let web_base_url = std::env::var("WEB_BASE_URL").ok().filter(|s| !s.is_empty());
...
pub struct Config {
    ...
    /// Nostr relay URLs to query (wss:// or ws://). No default — must be configured.
    #[serde(default)]
    pub relays: Vec<String>,
    /// Trusted publisher pubkeys (hex). Defaults to the U2P network publisher;
    /// set to [] to accept any valid-signature event.
    #[serde(default = "default_allowed_pubkeys")]
    pub allowed_pubkeys: Vec<String>,
    /// Optional base URL for a per-result details link (e.g. a web UI). Empty = no link.
    #[serde(default)]
...
fn default_allowed_pubkeys() -> Vec<String> {
    vec!["6aeb55064ea8b777591055e5704612e0e863fcc00bb211741781be299473c54e".to_string()]
}
...
impl Default for Config {
    fn default() -> Self {
        Config {
            ...
            relays: Vec::new(),
            allowed_pubkeys: default_allowed_pubkeys(),
            web_base_url: None,
        }
    }
}
